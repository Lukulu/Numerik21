...
int main(int argc, char** argv)
{
	...
	if(alg_num == 2)
	{
		...
		double* new_r;
		double* Ap;
		new_r = (double*)malloc(sizeof(double)*n);
		r = (double*)malloc(sizeof(double)*n);
		p = (double*)malloc(sizeof(double)*n);
		for (int i = 0; i < n; i++) {
			r[i] = b[i];
		...
		while (k < K)
		{
			...
			double alpha = euclidean_scp(r,r,n) / euclidean_scp(Ap,p,n);
			for (int i = 0; i < n; i++)
			{
				x[i] += alpha*p[i];
				new_r[i] = r[i] - alpha*Ap[i];
				p[i] = new_r[i] + euclidean_scp(new_r,new_r,n)/euclidean_scp(r,r,n)*p[i];
				P[k*K+i] = p[i];
				R[k*K+i] = new_r[i];
				r[i] = new_r[i];
			}
