...
int main(int argc, char** argv)
{
  ...
  for (int j = 0; j < n-2; j++) //n-2 iteration steps
  {
    ...
    double v2 = sqrt(0.5*(1 + fabs(A[j*n+(j+1)])/abs)); //second entry HH-vector
    v[j+1] = v2;
    // double sum = fabs(v2);
    ...
    for (int i = j+2; i < n; i++)
    {
      v[i] = sgn(A[j*n+i])*fabs(A[j*n+i])/2/v2/abs;
      // sum += fabs(v[i]);
    }
    ...
    for (int l = 0; l < n; l++) //iterate over columns of Q
    {
      ...
      for (int k = j; k < n; k++)
      {
        for (int d = j; d < n; d++) Q[k * n + l] += -2 * v[k] * v[d] * w[d];
      }
    }
