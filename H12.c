	...
	for (int j = 0; j <= findMin(n-1, m); j++)
	{
        ...
        //calculate Q
			    for (int d = j; d < m; d++)
			{
			    for (int k = j; k < m; k++)
			    {
			        if(k==d) Q[l*m+k] += -2/abs_v*v[k]*v[k]*y[k];
				    else     Q[l*m+k] += -2/abs_v*v[k]*v[d]*y[d];
				}
			}
		
		/*set vector x,y to the next column of A/Q*/
		    for (int k = 0; k < m; k++) x[k] = A[(l+1)*m+k];
			for (int k = 0; k < m; k++) y[k] = Q[(l+1)*m+k];
		}

		
		/*reset vector x to the first column of
		the new matrix A*/
		for (int k = 0; k < m; k++) x[k] = A[(j+1)*m+k];
